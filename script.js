...
function saveUsers(users) {
    ...
    // Ensure the admin account exists and has the correct credentials
...
function registerOrLogin() {
    ...
    if (user === ADMIN_USERNAME) {
        ...
        if (recent.length >= 3) {
            // compute lock start as time of the 3rd-from-last recent attempt
            const thirdFromLast = recent[recent.length - 3];
            const lockUntil = thirdFromLast.time + (60 * 60 * 1000); // 1 hour from that 3rd attempt
            if (now < lockUntil) {
...
function loadGroups() {
    ...
    groups.forEach(g => {
        ...
        const isMember = g.members && g.members.includes(u);
        ...
        div.innerHTML = `<strong>${privacyIcon} ${g.name}</strong>`;
...
function confirmRevealPassword(encodedUsername) {
    ...
    showConfirm(`¿Mostrar la contraseña de ${username}? Se revelará en texto plano.`, function(res) {
        if (!res) return;
        // find user and reveal password in the creds element
        const users = getUsers();
        const u = users.find(x => x.username === username);
        const el = document.getElementById('creds-' + encodedUsername);
        if (el) {
            const pass = u ? (u.password || '') : '';
            el.innerHTML = `Contraseña: <code>${pass}</code> <button onclick="hideRevealedPassword('${encodedUsername}')">Ocultar</button>`;
        }
    });
}
...
function hideRevealedPassword(encodedUsername) {
    const el = document.getElementById('creds-' + encodedUsername);
    if (!el) return;
    el.innerHTML = `Contraseña: <code>••••••</code> <button onclick="confirmRevealPassword('${encodedUsername}')">Mostrar</button>`;
}
